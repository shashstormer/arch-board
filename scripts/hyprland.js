...
function renderBindsTab() {
    ...
    const activeFlagCount = activeFlagFilters.size;

    let html = `
        <div class="search-container mb-6">
            <!-- Premium Header Toolbar -->
            <div class="flex flex-col bg-zinc-900/80 backdrop-blur-md border border-zinc-800/80 rounded-2xl p-4 shadow-xl">
                ...
                <div class="w-full border-t border-zinc-800/50 pt-3 mt-3 flex flex-col sm:flex-row items-center justify-between gap-3">
                    <div class="flex items-center gap-2 overflow-x-auto w-full no-scrollbar">
                        <span class="text-[10px] font-bold text-zinc-500 uppercase tracking-wider mr-1 whitespace-nowrap">Filter Flags:</span>
                        
                        ${flagOptions.map(opt => {
        const isActive = activeFlagFilters.has(opt.id);
        const activeClass = isActive
            ? 'bg-teal-500/10 text-teal-400 border-teal-500/50 shadow-sm shadow-teal-500/10'
            : 'bg-black/20 text-zinc-400 border-zinc-800 hover:bg-zinc-800 hover:text-zinc-200 hover:border-zinc-700';

        return `
                                <button onclick="toggleFlagFilter('${opt.id}')" 
                                        class="group px-3 py-1.5 text-[11px] font-medium rounded-lg border transition-all duration-200 flex items-center gap-2 ${activeClass}">
                                    ${isActive ? '<span class="w-1.5 h-1.5 rounded-full bg-teal-400 animate-pulse"></span>' : '<span class="w-1.5 h-1.5 rounded-full bg-zinc-700 group-hover:bg-zinc-600"></span>'}
                                    ${opt.label}
                                </button>
                            `;
    }).join('')}

                        ${activeFlagFilters.size > 0 ? `
                            <div class="w-px h-4 bg-zinc-800 mx-1"></div>
                            <button onclick="activeFlagFilters.clear(); renderTabContent('binds')" 
                                    class="px-2 py-1 text-[10px] font-bold text-red-400 hover:text-red-300 hover:bg-red-500/10 rounded-md transition-colors flex items-center gap-1">
